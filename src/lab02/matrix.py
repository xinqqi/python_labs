def transpose(mat: list[list[float | int]]) -> list[list]:  # транспонирование матрицы
    ...
    str_len=len(mat[0])                                     # проверка: если матрица «рваная» (строки разной длины) — ValueError
    for str in mat:
        if len(str)!=str_len:
            raise ValueError("")
    ...
    trans_mat = [[0] * num_str for _ in range(num_stolb)]   # создаем нулевую матрицу 

    for i in range(num_str):                               # меняем строки на столбцы, столбцы на строки
...
def row_sums(mat: list[list[float | int]]) -> list[float]:  # сумма строк матриц
    ...
    for i in range(num_str):
        sumsum=0                            
        for j in range(num_stolb):      # считаем сумму каждой строки матрицы и записываем в новый список
            sumsum += mat[i][j]
...
def col_sums(mat: list[list[float | int]]) -> list[float]:  # сумма столбцов матрицы
    ...
    for j in range(num_stolb):
        sumsum=0                            
        for i in range(num_str):        # считаем сумму каждого столбца матрицы и записываем в новый список
            sumsum += mat[i][j]
